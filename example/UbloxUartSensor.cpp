...
UbloxUartSensor::UbloxUartSensor(std::string name)
    : name(std::move(name)), _buffer(uart::sensors::UartSensor::DefaultReadBufferSize)
{
    resetTracking();
}

void UbloxUartSensor::resetTracking()
{
...
std::unique_ptr<uart::protocol::Packet> UbloxUartSensor::findPacket(uint8_t dataByte)
{
...
void UbloxUartSensor::packetFinderFunction(const std::vector<uint8_t>& data, const uart::xplat::TimeStamp& timestamp, uart::sensors::UartSensor::ValidPacketFoundHandler dispatchPacket, void* dispatchPacketUserData, void* userData)
{
...
uart::protocol::Packet::Type UbloxUartSensor::packetTypeFunction(const uart::protocol::Packet& packet)
{
    LOG_TRACE("called");
...
bool UbloxUartSensor::checksumFunction(const uart::protocol::Packet& packet)
{
    LOG_TRACE("called");
    ...
    if (packet.type() == uart::protocol::Packet::Type::TYPE_ASCII)
    {
        ...
        uint8_t checksumHex = checksumNMEA(packet.getRawData());
    ...
    {
        std::pair<uint8_t, uint8_t> checksum = checksumUBX(packet.getRawData());
...
bool UbloxUartSensor::isErrorFunction([[maybe_unused]] const uart::protocol::Packet& packet)
{
    LOG_TRACE("called");
...
bool UbloxUartSensor::isResponseFunction([[maybe_unused]] const uart::protocol::Packet& packet)
{
    LOG_TRACE("called");
