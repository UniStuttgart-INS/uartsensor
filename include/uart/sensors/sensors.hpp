...
class proglib_DLLEXPORT UartSensor : private util::NoCopy
{
  public:
    ...
    /// \brief Allows connecting to a uart sensor over a \ref uart::common::ISimplePort.
    ///
    /// The caller is responsible for properly destroying the
    /// \ref uart::common::ISimplePort object when this method is used. Also, if
    /// the provided \ref uart::common::ISimplePort is already open, then when
    /// the method \ref disconnect is called, \ref UartSensor will not attempt to
    /// close the port. However, if the \ref uart::common::ISimplePort is not
    /// open, then any subsequent calls to \ref disconnect will close the port.
    ///
    /// \param[in] port An \ref uart::xplat::IPort. May be either currently open or closed.
