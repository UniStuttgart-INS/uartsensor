...
struct SerialPort::Impl
{
    ...
    void open(bool checkAndToggleIsOpenFlag = true)
    {
        ...
        if (SerialPortHandle == INVALID_HANDLE_VALUE)
        {
            ...
            if (error == ERROR_ACCESS_DENIED)
            {
                ...
                throw std::logic_error("Port '" + PortName + "' already open.");
            }
    ...
    #elif __APPLE__
            O_RDWR | O_NOCTTY | O_NONBLOCK); // NOLINT
    #else
        #error "Unknown System"
    #endif
...
bool SerialPort_isFtdiUsbSerialPort(std::string portName)
{
...
HKEY SerialPort_getRegistryKeyForActiveFtdiPort(std::string portName, bool isReadOnly)
{
    HDEVINFO deviceInfoSet = SetupDiGetClassDevs(
        &GUID_DEVCLASS_PORTS,
        nullptr,
        ...
        &deviceData))
    {
        ...
        if (_tcscmp(mfgName, TEXT("FTDI")) != 0)
        {
            ...
            throw std::logic_error("invalid_operation");
        }
...
std::vector<std::string> SerialPort::getPortNames()
{
    ...
    #else
            comPorts.push_back(std::string(data));
    #endif
        }
    }
    ...
    {
        if (strstr(dirp->d_name, "tty.usbserial") != nullptr)
        {
            comPorts.emplace_back(std::string(dirp->d_name));
        }
    }
...
size_t SerialPort::NumberOfReceiveDataDroppedSections() // NOLINT
{
#if _WIN32
